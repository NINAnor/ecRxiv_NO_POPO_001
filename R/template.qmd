...
The pollinator potential indicator provides a spatial measure of ecosystem condition based on estimated pollinator potential and plant–pollinator relationships. Current indicator values vary among nature regions, with regional mean values ranging from $0.440$ in West to $0.458$ in North. The national mean is higher at $0.527$. These values represent pollinator potential relative to the reference conditions used to calibrate the indicator, rather than absolute pollinator abundance or population size. Spatial variation is also substantial: 13–28% of the area within the nature regions has indicator values $\ge 0.6$, with the highest proportion in South ($28.3\%$) and the lowest in West ($12.6\%)$; nationally, $32.3\%$ of the area is $\ge 0.6$.
...
Here, we also access data from biodiversity programs including the monitoring of semi-natural meadows (ASO) [@asoDataset], the national nature monitoring (ANO) [@anoDataOccurrence], and the Norwegian Insect Monitoring Program [@norwegianInsectMonitoring] from GBIF to validate the indicators. 
...
To account for regional differences in environmental conditions and species composition, reference sites are stratified into five biogeographic regions (corresponding to the [NaturIndeks]((https://www.naturindeks.no/Ecosystems/aapent_lavland)), hence we refer to the regions as nature regions hereafter): Central, East, North, South, and West. The reference condition therefore represents the range of pollinator potential expected under relatively favourable and stable conditions within each region, rather than an assumed historical or pristine baseline.
...
Reference levels varied substantially among the five nature regions, with the highest values in the East and the lowest in the North [@tbl-insect-indicator-reference-levels]. The upper reference level ($X_{100}$) ranged from $63$ in the North to $145$ in the East, while the lower reference level ($X_0$) was set to $0$ in all regions, representing the absence of pollinator potential. The number of reference locations also varied among regions, ranging from $473$ in the North to $1,184$ in the West.
...
Pollinator potential was substantially below the threshold value for good ecological condition (0.6) both nationally and in the five nature regions. East had the highest mean scaled indicator value (0.33), while Central had the lowest (0.27). South (0.32) also had a relatively high mean value, whereas West had a value closer to the national average. Across all open lowland areas, the national mean was 0.30 (SD = 0.20).

**This may change. Check it out after running the bootstrap.**

Pollinator potential varied among the five nature regions, although the differences in regional means were relatively small. North had the highest mean scaled indicator value ($0.458$), closely followed by East ($0.457$) and South ($0.456$), while West had the lowest value ($0.440$). Central had a similar value to West ($0.443$). At the national level, the mean indicator value was higher at $0.527$ ($SD = 0.0052$). In addition to these regional averages, the spatial distribution showed considerable variation: $12.6–28.3\%$ of the area within the five regions had indicator values $\ge 0.6$, with the highest proportion in South and the lowest in West. Nationally, $32.3\%$ of the area was $\ge 0.6$.
...
These are relative indicator scores, not percentages or counts of pollinators. Higher values indicate greater estimated pollinator potential relative to the reference conditions used to calibrate the indicator. The regional means therefore provide a basis for comparing ecosystem condition, while the proportion of area above 0.6 provides additional information on the spatial distribution of higher indicator values.

The small standard deviations ($0.0046–0.0063$) and narrow 95% bootstrap intervals indicate low uncertainty around the estimated regional means. The intervals ranged from $0.431–0.4464 in West to $0.451–0.466$ in North. Thus, the regional mean estimates are relatively precise, while the variation in indicator values across the landscape shows that regional averages alone do not fully describe the spatial pattern of pollinator potential.
...
The plant-host evaluation provides an additional ecological assessment of the pollinator potential indicator by examining its relationship with plant richness. Expected plant richness showed only a weak relationship with observed plant richness ($r < 0.18$; @fig-plantHostEvaluationPlot A), indicating limited agreement between modelled and observed plant richness at survey locations. In contrast, expected plant richness was positively related to the scaled pollinator potential indicator, with a stronger relationship in the ANO than in the ASO dataset (@fig-plantHostEvaluationPlot B). Pollinator potential showed a similar pattern, with a stronger relationship with expected plant richness in ANO than in ASO (@fig-plantHostEvaluationPlot C).
...
The strongest relationship was between expected plant richness and uncalibrated expected pollinator richness ($r > 0.94$; @fig-plantHostEvaluationPlot D). This strong relationship is expected because plant-host availability is directly incorporated into the estimation of pollinator richness. Overall, the results show that the pollinator potential indicator is related to plant-host richness. Care should therefore be taken when combining this indicator with other plant-host indicators, as they may capture overlapping ecological information and result in correlated measures of ecosystem condition.
...
The results indicate that the pollinator potential indicator provides a spatial measure of the capacity of open lowland ecosystems to support pollinator communities. Pollinator potential varied among nature regions, although regional mean values were relatively similar, ranging from 0.440 in West to 0.458 in North. The national mean was 0.527. There was also substantial spatial variation within regions, with 12.6–28.3% of the area having indicator values ≥0.6. This indicates that regional averages alone do not capture the full spatial variation in pollinator potential. Such variation is consistent with the understanding that pollinator communities respond to habitat, resource availability and landscape context, and that pollinators contribute to important ecosystem functions [@ollerton2017pollinator; @steffan2008interplay].

Evaluation against independent observations provides support for the ecological relevance of the indicator. Pollinator potential showed a strong positive relationship with observed insect richness ($r = 0.78$), indicating that areas with higher estimated pollinator potential tend to support richer insect communities. Its relationship with expected plant richness was weaker. The strong relationship between expected plant richness and uncalibrated expected pollinator richness ($r > 0.94$) is consistent with the important role of plant resources in supporting pollinator communities [@kral2021meta; @orford2016modest]. However, expected plant richness showed only a weak relationship with observed plant richness in this evaluation. The indicator should therefore not be interpreted as a direct measure of plant diversity, pollinator abundance, species richness, or realised pollination service. Pollination is an interaction-dependent ecosystem function, and its relationship with biodiversity cannot be represented by a single measure of species richness or by one trophic level alone [@loy2022effects; @bartholomee2019disentangling].

For management, the indicator is best interpreted as a relative measure of pollinator potential and ecosystem condition. Higher values indicate areas with greater estimated pollinator potential relative to the reference conditions used for calibration. The indicator can therefore help identify areas with comparatively high or low pollinator potential, support prioritisation for conservation or further investigation, and provide a basis for monitoring change over time. It should be interpreted alongside information on habitat, plant resources, land use and other pressures rather than being used alone to determine management actions. In particular, the indicator provides information about where conditions are more or less favourable for pollinators, rather than identifying the specific management action required at a location.

Calibration is important to this interpretation because it changes the indicator estimates and their position relative to the reference conditions. The magnitude and direction of the calibration effect varied among nature regions and between regional and national reference frameworks. This demonstrates that calibration is not simply a rescaling of the uncalibrated indicator, but an important step in relating pollinator potential to the ecological reference conditions against which ecosystem condition is assessed [@mcnellie2020reference]. The calibrated indicator should therefore be used when making condition assessments, with the selected reference framework clearly specified.

The relationship with plant-host richness also means that care is needed when combining the indicator with other plant-host indicators, as these measures may contain overlapping ecological information. The pollinator potential indicator should therefore be viewed as complementary to other biodiversity indicators rather than as an independent measure of all aspects of pollinator or plant community condition. Maintaining and restoring diverse and appropriately distributed plant resources is likely to be relevant for supporting pollinator communities, but management should consider the identity, availability and temporal continuity of resources as well as the wider habitat and landscape context [@kral2021meta; @orford2016modest; @loy2022effects].

Overall, the results support the use of the indicator to assess and compare pollinator potential across open lowland ecosystems in Norway. It captures ecologically meaningful spatial variation and shows a strong relationship with independent insect observations. The indicator is therefore sufficiently developed for spatial assessment and monitoring, while propagation of uncertainty from the underlying species-distribution models and plant–pollinator interaction data remains an important step towards full operationalisation.
